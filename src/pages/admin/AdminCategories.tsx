...
const empty = { id: undefined as string | undefined, name: "", slug: "", telugu_name: "", description: "", image_url: "", sort_order: 0, active: true };

export default function AdminCategories() {
  const { data, isLoading } = useAdminList("categories", "sort_order", true);
  ...
  return (
    <div>
      <PageHeader title="Categories" description="Top-level groupings shown on the home page."
        ...
        <div className="bg-card shadow-soft overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-muted/60 text-left text-xs uppercase tracking-wider">
              <tr><th className="p-3">Image</th><th className="p-3">Name</th><th className="p-3">Slug</th><th className="p-3">Order</th><th className="p-3">Status</th><th /></tr>
            </thead>
            <tbody>
              {(data ?? []).map((c: any) => (
                <tr key={c.id} className="border-t border-border">
                  <td className="p-3">{c.image_url && <img src={c.image_url} className="w-12 h-12 object-cover rounded" alt="" />}</td>
                  <td className="p-3 font-medium">{c.name}{c.telugu_name && <span className="block text-xs text-muted-foreground telugu">{c.telugu_name}</span>}</td>
              ...
              {!data?.length && <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">No categories.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
          ...
          {editing && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5"><Label>Telugu name</Label><Input className="telugu" value={editing.telugu_name ?? ""} onChange={(e) => setEditing({ ...editing, telugu_name: e.target.value })} /></div>
              </div>
              ...
              <ImageUploader value={editing.image_url} onChange={(u) => setEditing({ ...editing, image_url: u })} folder="categories" />
              <div className="grid grid-cols-2 gap-3">
